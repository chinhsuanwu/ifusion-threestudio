...
import threestudio
from threestudio.models.lora import inject_trainable_lora_extended
...
@threestudio.register("zero123-ifusion-guidance")
class Zero123iFusionGuidance(BaseObject):
    @dataclass
    class Config(BaseObject.Config):
        ...
        """Path to transform.json"""
        transform_fp: str = ""
        cond_image_paths: List[str] = field(default_factory=lambda: [])
        cond_elevation_degs: List[float] = field(default_factory=lambda: [])
        cond_azimuth_degs: List[float] = field(default_factory=lambda: [])
        cond_camera_distances: List[float] = field(default_factory=lambda: [])
    ...
    def configure(self) -> None:
        ...
        if self.cfg.lora.ckpt_path is not None:
            self.inject_lora(eval=True)
        ...
        self.prepare_embeddings(self.cfg.cond_image_paths)
        self.c_crossattns = torch.cat(self.c_crossattns)
        self.c_concats = torch.cat(self.c_concats)
    ...
    def load_transform(self, transform_fp: str):
        ...
        for i in range(len(meta["frames"])):
            ...
            self.cfg.cond_image_paths.append(fp)

            latlon = meta["frames"][i]["latlon"]

            # theta -90 -> bottom, theta 90 -> top in threestudio
            self.cfg.cond_elevation_degs.append(
                -latlon[0] + self.cfg.default_elevation_deg
            )
            self.cfg.cond_azimuth_degs.append(latlon[1])
            self.cfg.cond_camera_distances.append(latlon[2])

        # scale camera distances to avoid OOM
        scale_factor = (
            self.cfg.default_camera_distance / min(self.cfg.cond_camera_distances)
        )
        self.cfg.cond_camera_distances = [
            float(d * scale_factor) for d in self.cfg.cond_camera_distances
        ]

    @torch.cuda.amp.autocast(enabled=False)
    def prepare_embeddings(self, image_path) -> None:
        if OmegaConf.is_list(image_path):
            for path in image_path:
                self.prepare_embeddings(path)
            return
        ...
        c_crossattn, c_concat = self.get_img_embeds(self.rgb_256)
        print(f"[INFO] Loaded {image_path}!")
        self.c_crossattns.append(c_crossattn)
        self.c_concats.append(c_concat)
    ...
    def get_cond(
        ...
        **kwargs,
        ...
        # multi-view stochastic conditioning
        idx = torch.randint(
            len(self.cfg.cond_image_paths), (elevation.shape[0],)
        ).tolist()
        cond_elevation_deg = torch.tensor(
            [self.cfg.cond_elevation_degs[i] for i in idx], device=self.device
        )
        cond_azimuth_deg = torch.tensor(
            [self.cfg.cond_azimuth_degs[i] for i in idx], device=self.device
        )
        cond_camera_distance = torch.tensor(
            [self.cfg.cond_camera_distances[i] for i in idx], device=self.device
        )

        T = torch.stack(
            [
                torch.deg2rad(
                    (90 - elevation) - (90 - cond_elevation_deg)
                ),  # Zero123 polar is 90-elevation
                torch.sin(torch.deg2rad(azimuth - cond_azimuth_deg)),
                torch.cos(torch.deg2rad(azimuth - cond_azimuth_deg)),
                camera_distances - cond_camera_distance,
            ],
            dim=-1,
        )[:, None, :].to(self.device)
        cond = {}
        ...
        cond["c_concat"] = [
            torch.cat(
                [
                    torch.zeros_like(self.c_concats[idx]).to(self.device),
                    self.c_concats[idx],
                ],
                dim=0,
            )
    ...
    @torch.no_grad()
    def inject_lora(self, eval: bool = False):
        threestudio.info(
            f"Injecting LoRA from "
            + (
                str(self.cfg.lora.ckpt_path)
                if self.cfg.lora.ckpt_path is not None
                else "scratch"
            ),
        )
        lora_params, _ = inject_trainable_lora_extended(
            self.model.model,
            target_replace_module=set(self.cfg.lora.target_replace_module),
            r=self.cfg.lora.rank,
            loras=self.cfg.lora.ckpt_path,
            eval=eval,
        )
        return self
